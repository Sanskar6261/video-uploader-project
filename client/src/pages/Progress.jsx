        ...
        await api
          .post("/files/upload", fd, {
            ...
            onUploadProgress: (e) => {
              ...
              if (loaded === total && !uploadEndRef.current) {
                uploadEndRef.current = Date.now();
                setPhase("finalizing");
          ...
          .then((res) => {
            const endMs = uploadEndRef.current ?? Date.now();
            const elapsedSec = Math.max(
              0,
              Math.round((endMs - startRef.current) / 1000)
            );

            setUploaded({ ...res.data, uploadTime: elapsedSec });

            setPhase("done");
            setPct(100);

            navigate("/done", { state: { uploadTime: elapsedSec } });
          });
      } catch (e) {
        console.error(e);
        setPhase("error");
